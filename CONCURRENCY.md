...
### OrderBook Mutex
The `OrderBook` is guarded by its own shared mutex unique to that object's instance. This lock is a `shared_mutex` which allows any thread to concurrently hold a "shared" read lock while write operations are still exclusive to one thread which prevents concurrent modifications. This design ultimately allows for an easier approach of preventing double-accessing of the orderbook while allowing for future improvements down the line. One important dis-advantage to this strategy is that if a class were to first acquire the lock for the orderbook, then the lock for the transition queue, this could create a deadlock between the two. As a result, all concurrent tasks must be run from the `Simulator` class as it prevents this deadlock condition and is more logical in the scope of the entire order book.

### Stop Method
The stop method simply calls the `request_stop()` method on both the producer and consumer thread. This method was greatly simplified through the implementation of `jthread` which gets rid of the need for `join()` method calls on each thread as that call is made in the destructor of the `jthread`. The `request_stop()` method changes the stop token to True, allowing both threads to exit their respective loops.

### Tradeoffs
The first tradeoff made was deciding to use `jthread` and a `stop_token` instead of a regular `thread` and `atomic` variables. This was mainly done to take advantage of `jthread`'s automatic joining when the `stop()` function is called. Furthermore this ensures that the program fully takes advantage of all the new C++ 20 features. Another tradeoff is using `condition_variable_any` instead of `condition_variable` which was mainly done due to compatibility with `jthread`. The third tradeoff that was made was deciding the number of consumer threads. Having a pool of consumer threads would complicate all writes to the `transitionQueue` as there would need to be a significantly increased amount of communication among threads. Despite this benefit, having more threads would increase realism and expected load.

### Race Conditions
The first race condition that was accounted for is stopping all the threads when the consumer thread holds the mutex. The `wait()` method is utilized to ensure that the mutex will continue to be held if the `transitionQueue` is not empty and a stop has not been requested. This ensures that the lock is never acquired while a stop has been requested. Another race condition that was accounted for was ensuring only one thread can directly read or write to the `transitionQueue`. This is solved with a unique lock ensuring only one thread can access the queue at a time.

### Future Plans
Some future plans that will be implemented are multiple producer threads to increase the pressure that the OrderBook and Matching Engine face. In addition, implementing a lock-free queue would help reduce latency in the OrderBook by ensuring threads don't need to wait for each other.